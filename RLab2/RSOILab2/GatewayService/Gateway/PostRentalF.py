...
from .Req import delete_data_from_service, post_data_from_service

PostRentalFinishB = Blueprint('PostRentalFinish', __name__, )


@PostRentalFinishB.route('/api/v1/rental/<string:rentalUid>/finish', methods=['POST'])
async def post_rental_finish(rentalUid: str) -> Response:
    ...
    if response is None:
        return Response(
            ...
            response=json.dumps({
                'errors': ['Error the service unavailable.']
            })
        )
